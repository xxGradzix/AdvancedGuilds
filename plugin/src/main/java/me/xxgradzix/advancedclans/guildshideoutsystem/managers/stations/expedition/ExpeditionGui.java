...
public class ExpeditionGui {
    ...
    private static void ventureChooseGui(Player player) {
        ...
        for(ExpeditionVariant variant : variants) {
            for (int i = 0; i<2; i++) {
                GuiItem item = new GuiItem(ExpeditionItemManager.createObjectiveGuiItem(variant, i));
    ...
    private static void venturePreparation(Player player, ExpeditionVariant variant) {
        ...
        gui.setDefaultClickAction(event -> {
            ...
            switch (event.getAction()) {
                case PLACE_ALL, PLACE_ONE, PLACE_SOME, SWAP_WITH_CURSOR-> {
                    ...
                    if(event.getSlot() == toolSlot) {
                        if(!item.isSimilar(ExpeditionItemManager.toolTier1) && !item.isSimilar(ExpeditionItemManager.toolTier2) && !item.isSimilar(ExpeditionItemManager.toolTier3)) {
                            event.setCancelled(true);
                            break;
                        }
                    }
                    if(event.getSlot() == foodSlot) {
                        if(!item.isSimilar(ExpeditionItemManager.foodRationTier3) && !item.isSimilar(ExpeditionItemManager.foodRationTier2) && !item.isSimilar(ExpeditionItemManager.foodRationTier1)) {
                            event.setCancelled(true);
                            break;
                        }
                    }
            ...
            if(event.getSlot() == toolSlot || event.getSlot() == foodSlot || (event.getClickedInventory() != null && !event.getClickedInventory().equals(gui.getInventory()))) {
                ...
                Bukkit.getScheduler().runTaskLaterAsynchronously(AdvancedGuilds.instance, () -> {
                    ...
                    if(foodItem != null) {
                        if(foodItem.isSimilar(ExpeditionItemManager.foodRationTier1)) {
                            foodTier.set(1);
                        } else if(foodItem.isSimilar(ExpeditionItemManager.foodRationTier2)) {
                            foodTier.set(2);
                        } else if(foodItem.isSimilar(ExpeditionItemManager.foodRationTier3)) {
                            foodTier.set(3);
                        } else {
                            foodTier.set(0);
                        }
                    ...
                    if (toolItem != null) {
                        if(toolItem.isSimilar(ExpeditionItemManager.toolTier1)) {
                            toolTier.set(1);
                        } else if(toolItem.isSimilar(ExpeditionItemManager.toolTier2)) {
                            toolTier.set(2);
                        } else if(toolItem.isSimilar(ExpeditionItemManager.toolTier3)) {
                            toolTier.set(3);
                        } else {
                            toolTier.set(0);
                        }
                    ...
                    gui.updateItem(23, ExpeditionItemManager.getDifficultyItem(chance.get()));
                    gui.updateItem(24, ExpeditionItemManager.getStartExpeditionItem(chance.get(), variant.getLevel(), variant.getObjective(), variant.getCooldownSeconds()));
                    gui.updateItem(25, ExpeditionItemManager.getStartExpeditionItem(chance.get(), variant.getLevel(), variant.getObjective(), variant.getCooldownSeconds()));

                    gui.updateItem(toolSlot+1, new GuiItem(ExpeditionItemManager.createToolBonusSuppliedItem(toolTier.get())));
                    gui.updateItem(foodSlot+1, new GuiItem(ExpeditionItemManager.createFoodBonusSuppliedItem(foodTier.get())));
        ...
        GuiItem expeditionFood = new GuiItem(ExpeditionItemManager.createFoodBonusSuppliedItem(foodTier.get()));
        GuiItem expeditionTool = new GuiItem(ExpeditionItemManager.createToolBonusSuppliedItem(toolTier.get()));
        ...
        GuiItem startExpedition = new GuiItem(ExpeditionItemManager.getStartExpeditionItem(chance.get(), variant.getLevel(), variant.getObjective(), variant.getCooldownSeconds()));
        ...
        gui.setItem(23, new GuiItem(ExpeditionItemManager.getDifficultyItem(chance.get())));
    ...
    public static void openCurrentExpeditionGui(Player player) {
        ...
        GuiItem expeditionItem = new GuiItem(ExpeditionItemManager.getCurrentExpeditionItem(expeditionDto.getObjective(), expeditionDto.isFinished(), expeditionDto.getExpeditionLevel(), expeditionDto.secondsLeft()));

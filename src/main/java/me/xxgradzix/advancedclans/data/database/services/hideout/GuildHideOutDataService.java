...
public class GuildHideOutDataService {
    ...
    public GuildHideout resetOrCreateHideOut(String hideOutWorldName) {
        ...
        UpgradeInfoHolder storageUpgrade = new UpgradeInfoHolder((long) -1, true, GuildHideout.Upgrade.STORAGE);
        storageUpgrade.setFinished();

        guildHideout.setUpgradeHolder(GuildHideout.Upgrade.STORAGE, storageUpgrade);
        try {
    ...
    public void refreshHideoutOutpostHolograms(GuildHideout guildHideout, Location location) {
        ...
        if(guildHideout.getClanTag() == null || guildHideout.getClanTag().isEmpty()) {
            ...
            paste(location, GuildHideout.Upgrade.OUTPOST_PODEST.getSchemFile());
        } else {
            ...
            paste(location, GuildHideout.Upgrade.OUTPOST_HUT.getSchemFile());
        }
    }
    ...
    private void scheduleUpgrade(GuildHideout hideout, GuildHideout.Upgrade upgrade) {
        UpgradeInfoHolder upgradeHolder = hideout.getUpgradeHolder(upgrade);
    ...
    public void resetNpcsForHideOut(GuildHideout guildHideout) {
        ...
        if(guildHideout.hasFinishedUpgrade(GuildHideout.Upgrade.ILLEGAL_BUSINESS_HALL)) {
    ...
    private void setHolograms(@NotNull GuildHideout guildHideout) throws InvalidObjectException {
        ...
        if(!guildHideout.hasBoughtUpgrade(GuildHideout.Upgrade.ILLEGAL_BUSINESS_HALL)) {
    ...
    public void loadHideOuts() {
        ...
        for (GuildHideout guildHideout : guildHideouts) {
            HashMap<GuildHideout.Upgrade, UpgradeInfoHolder> hideoutUpgrades = guildHideout.getHideoutUpgrades();

            for (Map.Entry<GuildHideout.Upgrade, UpgradeInfoHolder> entry : hideoutUpgrades.entrySet()) {
                GuildHideout.Upgrade upgrade = entry.getKey();
    ...
    public void resetHideOutCompletelyOrCreate(String worldName) throws InvalidObjectException, InvalidHideoutWorldNameException {
        ...
        paste(loc, GuildHideout.Upgrade.RESET.getSchemFile());
        prepareHideOutHologramsAndNpcs(guildHideout);
    }
